...
_DEFAULT_PROJECT = "my-project-mnad-gis" # ID проекта в Google Cloud

ee.Authenticate() # Авторизация в GEE
...
def init_ee(project: str | None = None) -> None:
    load_dotenv() # Подхватывает переменные из .env
    
    _setup_ssl_for_macos() # Включает "патч" сертификатов
    
    # Выбираем ID проекта Google Cloud. 
    # Приоритет: аргумент функции -> файл .env -> значение по умолчанию.
    proj = project or os.environ.get("EE_PROJECT") or _DEFAULT_PROJECT
    
    ee.Initialize(project=proj) # Подключаемся к серверам Google Earth Engine под выбранным проектом
